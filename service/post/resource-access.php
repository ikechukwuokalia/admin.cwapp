...
if( $params ):
  if( !empty($params['id']) && !$access = (new MultiForm(get_database("data", \IO\get_constant("PRJ_SERVER_NAME")),'resource_access','id'))->findById($params['id']) ){
        ...
        <div class="paddn -pall -p20">
          ...
          <?php }else{ ?>
                ...
                <div class="grid-6-tablet">
                  ...
                  <select name="group_name" id="post-group-name" required>
                    ...
                    <optgroup label="Groups">
                      <?php if ($groups = (new MultiForm(get_database("data", \IO\get_constant("PRJ_SERVER_NAME")), "access_types", "name", $database))->findBySql("SELECT * FROM :db:.:tbl: WHERE `rank` > 1 AND `name` NOT IN (SELECT `group_name` FROM :db:.resource_access WHERE `resource` = '{$database->escapeValue($params['resource'])}') ORDER BY `rank` ASC")) {
            ...
            <div class="grid-12-tablet">
              ...
              <?php if ($scopes = (new MultiForm(get_database("data", \IO\get_constant("PRJ_SERVER_NAME")), "access_scopes", "name", $database))->findBySql("SELECT * FROM :db:.:tbl: ORDER BY `rank`")) {
