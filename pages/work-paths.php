            ...
            <div class="grid-5-tablet">
              ...
              <select name="domain" id="query-domain">
                ...
                <optgroup label="Work domains">
                  <?php if ($domains = (new MultiForm(get_database("CWS", "admin"), "work_domains", "name"))->findBySql("SELECT * FROM :db:.:tbl: WHERE `name` IN ( SELECT `domain` FROM :db:.work_paths) ORDER BY `name` ASC ")) {
            ...
            <div class="grid-4-tablet">
              ...
              <select name="task_group" id="query-task-group">
                ...
                <optgroup label="Task groups">
                  <?php if ($tgs = (new MultiForm(get_database("CWS", "data"), "task_groups", "name"))->findBySql("SELECT * FROM :db:.:tbl: WHERE `name` IN ( SELECT `task_group` FROM `{$adm_db}`.work_paths) ORDER BY `name` ASC ")) {
                    foreach ($tgs as $dmn) {
                      echo "<option value=\"{$dmn->name}\"";
