...
$nav_group = "index";
$page_name = "setting-options";
\check_access("READ", "/setting-options", $work_domain,"", true);
// get default API app
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <title>Setting options | <?php echo get_constant("PRJ_TITLE"); ?></title>
  <meta charset="UTF-8">
...
<body class="theme-<?php echo \IO\get_constant("PRJ_THEME"); ?>">
  ...
  <input type="hidden" data-setup="page" data-pager="#data-pager" data-container="#options-list" data-search="options" data-handler="lsStOpt" data-name="<?php echo $page_name; ?>" data-group="<?php echo $nav_group; ?>">
  ...
  <section id="cwos-content">
    <form id="patch-opt-form" method="post" action="/app/admin/src/PatchSettingOption.php" data-validate="false" onsubmit="cwos.form.submit(this,checkPost); return false;" >
      <input type="hidden" name="form" value="patch-opt-form">
      <input type="hidden" name="CSRF_token" value="<?php echo $session->createCSRFtoken("patch-opt-form");?>">
      ...
      <br class="c-f">
        <div class="grid-10-laptop grid-8-desktop center-laptop">
          <form
            ...
            method="post"
            action="/app/admin/src/GetSettingOption.php"
            data-validate="false"
            onsubmit="cwos.form.submit(this, doFetch); return false;"
            >
            <input type="hidden" name="form" value="option-query-form">
            <input type="hidden" name="CSRF_token" value="<?php echo $session->createCSRFtoken("option-query-form");?>">

            <div class="grid-6-tablet">
              <input type="checkbox" name="disabled" value="1" id="show-disabled">
              <label for="show-disabled">Include disabled items</label>
            </div>
            <br class="c-f">
    ...
    <div id="actn-btns">
      <div id="actn-btn-wrp">
        <div id="scrl-wrp">
          <button class="theme-button asphalt block" onclick="cwos.faderBox.url('/app/admin/post/setting-option', {callback : 'requery'}, {exitBtn: true});"> <i class="fas fa-plus fa-lg"></i> New Option</button>
        </div>
      </div>
