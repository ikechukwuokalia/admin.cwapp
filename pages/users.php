...
\require_login(true);
// \check_access("/users", true, "project-dev");
$work_domain = "project-admin";
$nav_group = "index";
$page_name = "users";
\check_access("READ", "/users", $work_domain,"", true);
// get default API app
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <title>Admin Users | <?php echo get_constant("PRJ_TITLE"); ?></title>
  <meta charset="UTF-8">
  ...
  <!-- Project styling -->
  <link rel="stylesheet" href="/app/cataliwos/plugin.cwapp/css/fancybox.min.css">
...
<body class="theme-<?php echo \IO\get_constant("PRJ_THEME"); ?>">
  ...
  <input type="hidden" data-setup="page" data-pager="#data-pager" data-container="#users-list" data-search="users" data-handler="listUser" data-name="<?php echo $page_name; ?>" data-group="<?php echo $nav_group; ?>">
  ...
  <section id="cwos-content">
      ...
      <br class="c-f">
        <div class="grid-10-laptop grid-8-desktop center-laptop">
          <form
            ...
            method="post"
            action="/app/admin/src/GetUser.php"
            data-validate="false"
    ...
    <div id="actn-btns">
      <div id="actn-btn-wrp">
        <div id="scrl-wrp">
          <button class="theme-button ashalt block" onclick="cwos.faderBox.url('/app/admin/put/user', {callback : 'requery'}, {exitBtn: true});"> <i class="fas fa-user-plus fa-lg"></i> New User</button>
        </div>
      </div>
    ...
    <script src="/app/cataliwos/plugin.cwapp/js/fancybox.min.js"></script>
